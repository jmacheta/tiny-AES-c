...
static uint8_t const Rcon[11] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
...
void AES_init_ctx(struct AES_ctx *ctx, uint8_t const *key) { KeyExpansion(ctx->RoundKey, key); }
#if(defined(CBC) && (CBC == 1)) || (defined(CTR) && (CTR == 1))
void AES_init_ctx_iv(struct AES_ctx *ctx, uint8_t const *key, uint8_t const *iv) {
  KeyExpansion(ctx->RoundKey, key);
  memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}
void AES_ctx_set_iv(struct AES_ctx *ctx, uint8_t const *iv) { memcpy(ctx->Iv, iv, AES_BLOCKLEN); }
#endif
...
#if(defined(CBC) && CBC == 1) || (defined(ECB) && ECB == 1)
static void InvCipher(state_t *state, uint8_t const *RoundKey) {
  uint8_t round = 0;
...
static void XorWithIv(uint8_t *buf, uint8_t const *Iv) {
  uint8_t i;
